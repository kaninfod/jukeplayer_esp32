...
class JukeBoxApp:
    ...
    def __init__(self):
        ...
        # SPI2/VSPI for Display - COMMENTED OUT (TODO: Replace with 0.96" OLED display when available)
        # self.spi_display = SPI(display_cfg.get("spi_unit", 2), 
        ...
        #                        miso=Pin(display_cfg.get("miso", 12)))
        
        # Initialize components with separate SPI buses - pass dummy_mode to all
        # DISPLAY COMMENTED OUT - Focus on WebSocket first
        # self.display = Display(self.spi_display, 
        #                        dc_pin=display_cfg.get("dc", 2), 
        #                        reset_pin=display_cfg.get("reset", 12), 
        #                        cs_pin=display_cfg.get("cs", 15), 
        #                        dummy_mode=self.dummy_mode)
        
        # Initialize NFC with error handling (hardware may not be connected)
        try:
        ...
        self.audio_handler = HttpAudioStreamHandler(self.config, control_ws=self.ws)
    ...
    async def _websocket_loop(self):
        ...
        while True:
            ...
            if self.oled:
                self.oled.set_text("Reconnecting...")
    ...
    async def _handle_websocket_message(self, data):
        ...
        try:
            ...
            if msg_type == "register_response":
                ...
                log.info(f"Track update - {title} (status: {status})")
